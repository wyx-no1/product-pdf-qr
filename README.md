...
生产运行面配置与仅限合成数据的验收入口见
[`docs/production-runtime-pr1.md`](docs/production-runtime-pr1.md)。真实服务器、
DNS、证书与发布批准不在本仓库 Agent 的执行范围内。

> [!CAUTION]
> **V1 已实现单管理员认证与生产运行面配置，但尚未完成真实基础设施部署、密钥注入和发布批准。仅允许在本机开发与 CI 的合成数据环境中验收，未经完整发布门禁不得对外暴露。**

本仓库当前实现 V1 最小业务闭环：管理员登录、创建与搜索产品、Excel 批量导入产品、上传并安全校验 PDF、追加当前版本、生成永久二维码，以及通过公开随机地址访问当前 PDF。产品启停、历史恢复和 Excel 导入管理入口均已实现；生产部署仍属后续阶段。
...
6. 从产品列表进入“Excel 批量导入产品”，上传首行为表头的 `.xlsx`，并核对成功新增、重复跳过、格式错误及完整错误清单。
...
└── domains/
    ...
    ├── importer/          # XLSX 安全检查、两阶段校验/写入与三类计数
