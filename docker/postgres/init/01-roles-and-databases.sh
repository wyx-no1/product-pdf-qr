...
for database_name in "$APP_DATABASE_NAME" "$APP_TEST_DATABASE_NAME"; do
...
CREATE DATABASE "$APP_TEST_DATABASE_NAME" OWNER app_migrate;
REVOKE CONNECT, TEMPORARY ON DATABASE "$APP_DATABASE_NAME" FROM PUBLIC;
REVOKE CONNECT, TEMPORARY ON DATABASE "$APP_TEST_DATABASE_NAME" FROM PUBLIC;
GRANT CONNECT ON DATABASE "$APP_DATABASE_NAME" TO app_migrate, app_rw, app_backup;
GRANT CONNECT ON DATABASE "$APP_TEST_DATABASE_NAME" TO app_migrate, app_rw, app_backup;
SQL
